...
class PythonRenderer(BaseRenderer):
    ...
    def render_module(self, module: ModuleSpec) -> str:
        ...
        parts.append("")
        parts.append("Auto-generated by logic2code from Code2Logic output.")
        parts.append('"""')
        parts.append("")
    ...
    def _collect_imports(self, module: ModuleSpec) -> str:
        ...
        has_async = has_async or any(any(m.is_async for m in cls.methods) for cls in module.classes)
        if has_async:
    ...
    def _render_function_internal(self, func: FunctionSpec, is_method: bool = False) -> str:
        ...
        if self.config.include_type_hints and func.return_type:
            ...
            if (
                func.is_async
                and not ret.startswith("Coroutine")
                and not ret.startswith("Awaitable")
            ):
                return_type = f" -> {ret}"
            else:
                return_type = f" -> {ret}"
