...
class HybridDataAnalyzer:
    ...
    def analyze_data_hubs_and_consolidation(self) -> Dict:
        ...
        for func_name, func_data in consolidated_data.items():
            if isinstance(func_data, dict):
                ...
                G.add_node(
                    func_name,
                    types=func_types,
                    connections=len(func_data.get("calls", [])),
                )
        ...
        high_pagerank = {
            node: score for node, score in pagerank.items() if score > 0.02
        }
                ...
                for pattern_type, pattern_info in patterns.items():
                    if pattern_info:
                        process_patterns[pattern_type].append(
                            {"function": func_name, "pattern": pattern_info}
                        )
...
"""
        ...
        return {
            ...
            "total_redundant_functions": sum(
                len(funcs) for funcs in redundant_processes.values()
            ),
                ...
                unification_opportunities.append(
                    {
                        ...
                        "size": len(community),
                        "most_connected": type_freq.most_common(1)[0]
                        if type_freq
                        else None,
...
"""
    ...
    def detect_data_flow_cycles(self) -> Dict:
        ...
        for cycle in cycles:
            if len(cycle) > 3:  # Long cycles
                problematic_cycles.append(
                    {"cycle": cycle, "length": len(cycle), "issue": "long_cycle"}
                )
        ...
        multi_cycle_nodes = [
            (node, count) for node, count in node_cycle_count.items() if count > 1
        ]
...
"""
    ...
    def identify_unused_data_structures(self) -> Dict:
        ...
        unused_nodes = [
            node for node in usage_graph.nodes() if usage_graph.in_degree(node) == 0
        ]

        # Find dead ends (no outgoing edges)
        dead_ends = [
            node for node in usage_graph.nodes() if usage_graph.out_degree(node) == 0
        ]

        # Find isolated nodes (no edges at all)
        isolated_nodes = [
            node for node in usage_graph.nodes() if usage_graph.degree(node) == 0
        ]
...
"""
    ...
    def quantify_process_diversity(self) -> Dict:
        ...
        high_diversity = {
            dt: metrics
            for dt, metrics in diversity_metrics.items()
    ...
    def trace_data_mutations_patterns(self) -> Dict:
                ...
                for mutation_type, mutation_info in mutations.items():
                    if mutation_info:
                        mutation_patterns[mutation_type].append(
                            {"function": func_name, "pattern": mutation_info}
                        )
        ...
        for func_name, func_data in all_functions.items():
            if isinstance(func_data, dict):
                mutation_count = sum(
                    len(m)
                    for m in self._identify_mutation_patterns(
                        func_name, func_data
                    ).values()
                )
                if mutation_count > 3:
                    high_mutation_functions.append(
                        {"function": func_name, "mutation_count": mutation_count}
                    )
...
"""
        ...
        return {
            ...
            "total_mutations": sum(
                impact["count"] for impact in mutation_impact.values()
            ),
    ...
    def score_data_complexity_hotspots(self) -> Dict:
        ...
        complexity_stats = {
            "mean": sum(complexity_scores.values()) / len(complexity_scores)
...
"""
    ...
    def generate_type_reduction_plan(self) -> Dict:
        ...
        generic_types = [
            t for t in type_usage.keys() if t in ["dict", "list", "str", "int"]
        ]
...
"""
    ...
    def analyze_inter_module_dependencies(self) -> Dict:
            ...
            if isinstance(func_data, dict):
                ...
                if "calls" in func_data:
                    for called in func_data["calls"][:10]:
                        called_module = (
                            called.rsplit(".", 1)[0] if "." in called else "root"
                        )
        ...
        dependency_metrics = {
            ...
            "max_out_degree": max(dict(module_graph.out_degree()).values())
            if module_graph.nodes()
            else 0,
            "max_in_degree": max(dict(module_graph.in_degree()).values())
            if module_graph.nodes()
            else 0,
        }
        ...
        central_modules = sorted(centrality.items(), key=lambda x: x[1], reverse=True)[
            :5
        ]
        ...
        tightly_coupled = sorted(
            coupling.items(), key=lambda x: x[1]["total"], reverse=True
        )[:5]
...
"""
    ...
    def _load_all_functions(self) -> Dict:
        ...
        if orphans_dir.exists():
            for file_path in orphans_dir.glob("functions_*.yaml")[
                :10
    ...
    def _identify_process_patterns(
        self, item_name: str, item_data: Dict
    ) -> Dict[str, Any]:
                ...
                opportunities.append(
                    {
                        ...
                        "total_usage": sum(
                            len(data_types[t]) for t in similar_types if t in data_types
                        ),
                    }
                )
    ...
    def _format_consolidation_opportunities(self, opportunities: List[Dict]) -> str:
        ...
        for opp in opportunities:
            result.append(
                f"  • {opp['base_type']}: {len(opp['similar_types'])} podobnych typów"
            )
            result.append(f"    Typy: {', '.join(opp['similar_types'][:3])}")

        return "\n".join(result)

    def _get_top_files_to_modify(
        self, hubs: Dict, high_pagerank: Dict, limit: int
    ) -> str:
    ...
    def _format_unification_opportunities(self, opportunities: List[Dict]) -> str:
        ...
        for opp in opportunities:
            result.append(
                f"  • Społeczność {opp['community_id']}: {len(opp['types'])} typów"
            )
    ...
    def _get_cycle_breaking_suggestions(
        self, cycles: List[Dict], nodes: List[Tuple]
    ) -> str:
        ...
        for cycle in cycles[:2]:
            suggestions.append(
                f"  • Przerwij cykl {cycle['cycle'][0]} -> {cycle['cycle'][-1]}"
            )
    ...
    def _format_removal_candidates(self, candidates: List[Dict]) -> str:
        ...
        for candidate in candidates[:5]:
            result.append(
                f"  • {candidate['name']}: złożoność {candidate['complexity']}"
            )
    ...
    def _identify_mutation_patterns(
        self, func_name: str, func_data: Dict
    ) -> Dict[str, List]:
        """Identify mutation patterns."""
        patterns = {
            "list_mutations": [],
        ...
        if any(
            word in name_lower
            for word in ["append", "extend", "insert", "remove", "pop"]
        ):
            patterns["list_mutations"].append("list_mutation")

        if any(
            word in name_lower for word in ["update", "popitem", "setdefault", "clear"]
        ):
  ...
  • Powtarzający kod -> funkcja pomocnicza"""
    ...
    def _format_similar_types(self, similar_types: List) -> str:
        ...
        for similar in similar_types[:3]:
            result.append(
                f"  • {similar['base_type']}: {len(similar['similar_types'])} typów"
            )
    ...
    def _get_type_reduction_plan(
        self, rare_types: Dict, similar: List, generic: List
    ) -> str:
  ...
  • CustomClass1 + CustomClass2 -> BaseClass"""
    ...
    def _find_similar_types_for_merging(
        self, type_usage: Dict, type_locations: Dict
    ) -> List[Dict]:
    ...
    def _get_centralization_plan(
        self, central: List[Tuple], coupled: List[Tuple]
    ) -> str:
  ...
  • Stwórz shared context dla wspólnych danych"""
    ...
    def generate_summary_report(self):
        ...
        total_insights = sum(
            result.get("insights_count", 0) for result in self.analysis_results
        )
...
def main():
    ...
    print("Ready for LLM-based refactoring!")
